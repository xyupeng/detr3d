...
class CenterPoint(MVXTwoStageDetector):
    ...
    def extract_pts_feat(self, pts, img_feats, img_metas):
        ...
        # voxels: shape=(num_voxels, max_pts_per_voxel, pts_features)
        # num_points: shape=(num_voxels,); num_pts in each voxel
        # coors: shape=(num_boxels, 4); [batch_idx, z, y, x]

        voxel_features = self.pts_voxel_encoder(voxels, num_points, coors)  # shape=(num_voxels, num_features)
        batch_size = coors[-1, 0] + 1
        x = self.pts_middle_encoder(voxel_features, coors, batch_size)  # shape=(N, C * D, H, W); sparse to dense
        x = self.pts_backbone(x)  # x: tuple of multi-scale tensors; len(x) == num_stages
        if self.with_pts_neck:  # FPN: upsample at each stage, then concat all along channel dimension
            x = self.pts_neck(x)  # [tensor(shape=(N, C, H, W))]
        return x
